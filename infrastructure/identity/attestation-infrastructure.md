...
**¿Qué protocolos usan EAS realmente?**:

EAS es una infraestructura relativamente reciente y su adopción no es universal. Protocolos consolidados como [ENS](https://ens.domains/) o [Lens Protocol](https://lens.xyz/) tienen sus propios contratos independientes y no se basan en EAS: ENS gestiona la resolución de nombres con su propio Name Registry, y Lens modela el grafo social mediante NFTs de seguidores y publicaciones sin pasar por EAS. Estos protocolos surgieron antes de que EAS existiera o madurara, y construyeron su infraestructura propia con lógica específica que un sistema de attestations genérico no habría cubierto de forma natural.

La adopción de EAS en plataformas de reputación es real pero selectiva. Plataformas que sí han construido sobre EAS: [Gitcoin Passport](https://passport.gitcoin.co/) usa attestations EAS en Optimism y Base para cada stamp de identidad; [Optimism](https://optimism.io/) usa EAS en su programa RetroPGF para atestiguar contribuciones de builders al ecosistema; Coinbase lanzó sus [Coinbase Verifications](https://base.mirror.xyz/1UqAUpfcWAUuNjLxfNMNUCBLlFupHEsMZGTGeCq6yfg) sobre EAS en Base para verificaciones KYC; y plataformas especializadas en reputación de contribuidores DAO como [Karma](https://karmahq.xyz/) y [Clique](https://clique.social/) también emiten sobre EAS. En cambio, plataformas con requisitos más específicos optaron por infraestructura propia: Worldcoin necesita prueba de humanidad con hardware biométrico y no encaja en el modelo genérico de attestations; BrightID construye un grafo de confianza social off-chain con su propia lógica; DeBank calcula scores de reputación DeFi con contratos propios. El patrón que emerge es que EAS atrae a proyectos que necesitan emitir credenciales estandarizadas e interoperables, mientras que proyectos con mecánicas muy específicas de verificación tienden a construir su propia infraestructura.

**Emisores, confianza y modelo de consulta**:

Conviene dejar claro desde el principio un aspecto que genera confusión frecuente: EAS no es un catálogo de emisores. El Schema Registry cataloga estructuras de datos —los schemas— pero no tiene ningún registro de organizaciones ni personas autorizadas a emitir. Cualquier dirección Ethereum puede emitir attestations usando cualquier schema existente sin solicitar permiso ni registrarse en ningún lado. La autoridad de un emisor no la determina EAS, sino los propios protocolos que consumen las attestations, que deciden a qué emisores dar crédito y con qué peso.

El modelo operativo tampoco es de publicación y suscripción. No existe ningún mecanismo nativo por el que un consumidor se suscriba a un emisor y reciba notificaciones cuando este publica nuevas attestations. EAS funciona como un modelo de emisión y consulta: los emisores escriben attestations mediante transacciones on-chain, y los verificadores las consultan bajo demanda, ya sea directamente a los contratos EAS o a través de indexadores especializados como [The Graph](https://thegraph.com/). Esta arquitectura delega completamente en el consumidor la responsabilidad de construir confianza sobre el modelo abierto, algo que EAS no resuelve de forma intencionada. El análisis de cómo las aplicaciones abordan esta responsabilidad se desarrolla en el apartado *Tensiones y límites del modelo* más adelante.
