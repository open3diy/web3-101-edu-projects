...
Las Verifiable Credentials off-chain (estándar W3C) y las attestations on-chain (como EAS) comparten una base conceptual común pero difieren en sus mecanismos operativos. Este documento recoge las prácticas que aplican transversalmente a ambos modelos y donde las diferencias entre ellos son especialmente relevantes para diseñar sistemas correctos. Para profundizar en la arquitectura de cada uno, ver [did-protocols.md](did-protocols.md) y [attestation-infrastructure.md](attestation-infrastructure.md).

## La importancia crítica de confiar en el emisor

Determinar si debes confiar en el emisor de un VC o atestación es quizás el aspecto más importante y a menudo malentendido de las credenciales verificables.
...
## Diseño correcto: semántica temporal para attestations y VCs
...
**Credenciales con fecha de expiración explícita**:

"Trabajó en empresa X desde enero 2023 hasta diciembre 2023", donde la validez temporal está codificada en los datos del claim. Cuando alguien verifica la credencial en 2025, puede ver que ya expiró sin necesidad de consultar mecanismos de revocación. Esto funciona bien para contratos temporales, certificaciones que caducan, o membresías por tiempo limitado.

**Credenciales de estado actual con revocación**:

"Actualmente emplea a esta persona", que se entiende válida desde la emisión hasta que sea revocada explícitamente. El emisor tiene la responsabilidad de revocarla cuando el estado cambie. Esto es apropiado para relaciones continuas donde la fecha de finalización no se conoce al momento de emisión.

**Credenciales de eventos puntuales**:

"Completó este curso en marzo 2023" o "asistió a este evento en 2024", donde la afirmación describe un hecho histórico que no cambia con el tiempo y por tanto raramente requiere revocación. Estas son las credenciales más simples porque su validez es intrínsecamente permanente.
...
EAS implementa revocación nativa mediante la función `revoke()` documentada en [EAS: Revocation](https://docs.attest.sh/docs/core--concepts/revocation). Otras implementaciones de Attestation Layer pueden usar mecanismos similares con variaciones en los detalles técnicos.
...
La revocación plantea desafíos de privacidad importantes. Si cada verificación requiere consultar un registro centralizado controlado por el emisor, éste puede rastrear cuándo y dónde usas tus attestations. Cada consulta a la lista de revocación revela que alguien está verificando esa credencial específica en ese momento. Soluciones avanzadas como la [revocación basada en acumuladores criptográficos](https://eprint.iacr.org/2020/777) permiten verificar que una attestation no ha sido revocada sin revelar cuál attestation específica estás verificando, preservando la privacidad del titular. Estos sistemas usan estructuras criptográficas donde puedes probar que tu credencial no está en el conjunto de revocadas sin identificarla explícitamente.

## Referencias

- [EAS: Revocation](https://docs.attest.sh/docs/core--concepts/revocation) - Mecanismo nativo de revocación en EAS
- [W3C VC Data Model: Status](https://www.w3.org/TR/vc-data-model/#status) - Especificación de estado y revocación en VCs
- [Accumulator-based Revocation](https://eprint.iacr.org/2020/777) - Paper sobre revocación con preservación de privacidad
- [did-protocols.md](did-protocols.md) - Arquitectura DID y Verifiable Credentials
- [attestation-infrastructure.md](attestation-infrastructure.md) - EAS y Attestation Layer
